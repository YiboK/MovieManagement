...
public class MainRightFrame extends JFrame {

	private JPanel contentPane;
	private JDesktopPane desktopPane = null;
	private JTable movieRoleTable;
	private JTextField movieRoleTxt;
	private DbUtil dbUtil = new DbUtil();
	private JTable showRoleTable;
	private JTextField showRoleTxt;
	...
	public MainRightFrame() {
		...
		movieRoleTable = new JTable();
		movieRoleTable.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
		...
		movieRoleTable.getColumnModel().getColumn(0).setPreferredWidth(146);
		movieRoleTable.getColumnModel().getColumn(1).setPreferredWidth(147);
		movieRoleTable.setFont(new Font("Times New Roman", Font.PLAIN, 12));
		scrollPane.setViewportView(movieRoleTable);
		...
		JLabel lblNewLabel_1 = new JLabel("Look for Actors In");
		lblNewLabel_1.setFont(new Font("Baskerville Old Face", Font.BOLD, 18));
		lblNewLabel_1.setBounds(68, 185, 147, 20);
		desktopPane.add(lblNewLabel_1);
		
		movieRoleTxt = new JTextField();
		movieRoleTxt.setFont(new Font("Baskerville Old Face", Font.PLAIN, 18));
		movieRoleTxt.setBounds(225, 185, 113, 20);
		desktopPane.add(movieRoleTxt);
		movieRoleTxt.setColumns(10);
		...
		showRoleTable = new JTable();
		showRoleTable.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
				"PersonID", "Character Name"
		...
		showRoleTable.getColumnModel().getColumn(0).setPreferredWidth(163);
		showRoleTable.getColumnModel().getColumn(1).setPreferredWidth(153);
		showRoleTable.setFont(new Font("Times New Roman", Font.PLAIN, 12));
		scrollPane_1.setViewportView(showRoleTable);
		
		JLabel lblNewLabel_1_1 = new JLabel("Look for Actors In");
		lblNewLabel_1_1.setFont(new Font("Baskerville Old Face", Font.BOLD, 18));
		lblNewLabel_1_1.setBounds(1026, 189, 147, 20);
		desktopPane.add(lblNewLabel_1_1);
		
		JLabel lblNewLabel_2_1 = new JLabel("(TV Show)");
		lblNewLabel_2_1.setFont(new Font("Baskerville Old Face", Font.BOLD, 16));
		lblNewLabel_2_1.setBounds(1285, 188, 80, 20);
		desktopPane.add(lblNewLabel_2_1);
		
		showRoleTxt = new JTextField();
		showRoleTxt.setFont(new Font("Baskerville Old Face", Font.PLAIN, 18));
		showRoleTxt.setBounds(1183, 185, 92, 21);
		desktopPane.add(showRoleTxt);
		showRoleTxt.setColumns(10);
		...
		lblNewLabel_2.setFont(new Font("Baskerville Old Face", Font.BOLD, 16));
		lblNewLabel_2.setBounds(348, 189, 58, 15);
		desktopPane.add(lblNewLabel_2);
		
		JButton btnNewButton_3 = new JButton("Find Top Movie/Show");
		...
		btnNewButton_3.setBounds(68, 725, 253, 23);
	...
	private void RoleShowUpdateActionEvent(ActionEvent e) {
		String roleShow = showRoleTxt.getText();
		if (StringUtil.isEmpty(roleShow)) {
			JOptionPane.showMessageDialog(null, "Please Enter The Title of Any Show!");
			return;
		} else {
			this.fillTableShow(roleShow);
		}
		
	}

	private void RoleMovieUpdateActionEvent(ActionEvent e) {
		String roleMovie = movieRoleTxt.getText();
		if (StringUtil.isEmpty(roleMovie)) {
			JOptionPane.showMessageDialog(null, "Please Enter The Title of Any Movie!");
			return;
		} else {
			this.fillTableMovie(roleMovie);
		}
		
	}
	
	private void fillTableMovie(String MovieTitle) {
		DefaultTableModel dtm = (DefaultTableModel) movieRoleTable.getModel();
		dtm.setRowCount(0);
		Connection con = null;
		try {
			con = dbUtil.getCon();
			StringBuffer sb = new StringBuffer("call newimdb.getCastM('" + MovieTitle + "')");
	...
	private void fillTableShow(String ShowTitle) {
		DefaultTableModel dtm = (DefaultTableModel) showRoleTable.getModel();
		dtm.setRowCount(0);
		Connection con = null;
		try {
			con = dbUtil.getCon();
			StringBuffer sb = new StringBuffer("call newimdb.getCastT('" + ShowTitle + "')");
			PreparedStatement pstat = con.prepareStatement(sb.toString());
			ResultSet rs = pstat.executeQuery();
			while(rs.next()) {
				Vector v = new Vector();
